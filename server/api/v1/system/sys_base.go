...
// @Success 200 {object} response.Response
// @Router /system/base/createAliyunSecret [post]
func (a *BaseApi) CreateAliyunSecret(c *gin.Context) {
	var createAliyunSecret request.SaveAliyunSecret
	if err := c.ShouldBindJSON(&createAliyunSecret); err != nil {
		response.FailWithMessage("参数错误："+err.Error(), c)
		return
	}
